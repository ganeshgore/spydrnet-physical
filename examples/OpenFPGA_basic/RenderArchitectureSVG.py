...
# Mandatory arguments
parser.add_argument('--design_name', type=str, default="FPGA66_flex")
parser.add_argument('--arch_file', type=str,
                    default="example_files/vpr_arch_render_demo.xml")
...
parser.add_argument('--output_root', type=str, default="_release")
...
class FPGAShaping():
    ...
    def get_stype(self, x, y):
        '''
        Returns type of SB block.

        Inputs:
            x = x coordinate (column number)
            y = y coordinate (row number)
        Output:
            Integer from 0 to 8 relating to SB block type.

            SB Block Locations:
            0,FPGA_SIZE[1]   x,FPGA_SIZE[1]         FPGA_SIZE[0],FPGA_SIZE[1]
            ┌──────┐┌───┐┌─────────┐┌───┐┌──────┐
            │ 3    ││   ││    4    ││   ││   5  │
            │   ┌──┘└───┘└──┐   ┌──┘└───┘└──┐   │
            │   │┌─────────┐│   │┌─────────┐│   │
    ...
    def add_sb(self, xi, yi, Stype=None):
        '''
                   d
                 +----+
               c |    |
        ...
        Stype = Stype or self.get_stype(xi, yi)
        if Stype == 1:  # SB_0__0_
            llyB1 += c
            HeightB1 += -c
            llxB2 += b
