...
.. image:: ../../../../examples/OpenFPGA/clock_tree/_clock_tree_example_arch_03.svg
    :width: 220px
...
for x in range (int(WIDTH/4)+1 , WIDTH, int(WIDTH/2)+1):
    for y in range (int(HEIGHT/4)+2, HEIGHT, int(HEIGHT/2)-2):
        level1_patt.merge(p_manager.get_htree(int(WIDTH/2)).translate(xind, yind))
        yind = yind + 10
    yind = 3
    xind = xind + 10
...
for x in range (2):
    for y in range (2):
        ydir = -1 if y else 1
        pt = ConnectPoint(5+(x*(int(WIDTH/2)+1)), 8+(y*(int(WIDTH/2)+1))+ydir, 5+(x*(int(WIDTH/2)+1)), 8+(y*(int(WIDTH/2)+1))) 
