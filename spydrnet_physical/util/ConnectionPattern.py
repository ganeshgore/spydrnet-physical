...
class ConnectionPattern:
    ...
    def reset(self):
        ...
        self._connect = ConnectPointList(sizex=self.sizex, sizey=self.sizey)

    def get_fishbone(
        self, width=None, height=None, steps=1, x_margin=(0, 0), y_margin=(0, 0)
    ):
        """
    ...
    def render_pattern(self, scale=20, title=None, add_module_labels=False):
        ...
        if dwgMarker:
            ...
            for i in range(0, self.sizex + 1):
                dwgMarker.add(
                    dwg.line(
                        start=((i + 0.5) * scale, 0.5 * scale),
                        end=((i + 0.5) * scale, (self.sizey + 0.5) * scale),
                        class_="gridmarker",
                    )
                )
            for i in range(0, self.sizey + 1):
                dwgMarker.add(
                    dwg.line(
                        start=(0.5 * scale, (i + 0.5) * scale),
                        end=((self.sizex + 0.5) * scale, (i + 0.5) * scale),
                        class_="gridmarker",
                    )
                )
