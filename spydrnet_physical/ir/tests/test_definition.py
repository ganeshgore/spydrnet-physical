...
class TestDefinition(unittest.TestCase):
    ...
    def test_create_feedthroughs_ports(self):
        ...
        self.assertSetEqual(
            set(get_names(self.definition.get_ports())),
            {"cable1_feed_in", "cable1_feed_out"},
        )
        self.assertSetEqual(
            set(get_names(self.definition.get_cables())),
            {"cable1_feed_in", "cable1_feed_out"},
        )
    ...
    def test_create_feedthroughs_ports_2(self):
        ...
        def get_port_name(x):
            return "inport" if x is sdn.IN else "outport"

        port1, port2 = self.definition.create_feedthroughs_ports(
            cable, get_port_names=get_port_name
        )
        self.assertIsInstance(port1, sdn.Port)
        self.assertIsInstance(port2, sdn.Port)
        self.assertSetEqual(
            set(get_names(self.definition.get_ports())), {"inport", "outport"}
        )
    ...
    def test_create_feedthrough(self):
        ...
        for new_cable in new_cables:
            self.assertTrue(
                isinstance(new_cable, sdn.Cable), "Return value should be cable"
            )
        new_cables = new_cables[0]
        self.assertEqual(new_cables.size, 4, "New cable should have same dimensions")
        self.assertSetEqual(
            set(map(lambda p: p.name, module2.ports)), {"cable_ft_out", "cable_ft_in"}
        )
        self.assertSetEqual(
            set(map(lambda p: p.name, self.definition.get_cables())),
            {"cable", "cable_ft_in_0"},
        )
        self.assertSetEqual(
            set(("cable_ft_in_0", "cable")),
    ...
    def test_merge_instance(self):
        ...
        self.assertEqual(
            set(get_names(self.definition.get_instances())),
            {
                "def2_def3_merged_1",
            },
        )
        self.assertEqual(set(get_names(new_m.get_instances())), {"inst2", "inst3"})
    ...
    def test_flatten_instance(self):
        ...
        self.assertEqual(self.definition.children[0].name, "inst1_submodule_inst")
    ...
    def test_merge_multiple_instance(self):
        ...
        instance_list_tuple = [
            ((inst2, inst3), "merged_inst_2_3"),
            ((inst4, inst5), "merged_inst_4_5"),
        ]

        new_def, inst_list = self.definition.merge_multiple_instance(
            instance_list_tuple, new_definition_name="new_merged_inst"
        )
